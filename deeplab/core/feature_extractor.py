...
def _mobilenet_v2(net,
                  ...
                  scope=None,
                  final_endpoint=None):
  raise Exception('cannot be used')
...
arg_scopes_map = {
    'mobilenet_v2': None,    # mobilenet_v2.training_scope,
  ...
  if 'xception' in model_variant:
    ...
    features, end_points = get_network(
        model_variant, preprocess_images, arg_scope)(
            ...
            is_training=(is_training if fine_tune_batch_norm else False),
    ...
    arg_scope = arg_scopes_map[model_variant](
        is_training=(is_training if fine_tune_batch_norm else None),
        weight_decay=weight_decay)
    features, end_points = get_network(
        model_variant, preprocess_images, arg_scope)(
            inputs=images,
